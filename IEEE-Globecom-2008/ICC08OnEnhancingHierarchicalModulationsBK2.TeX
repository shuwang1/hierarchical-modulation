...
relatively little attention to date. Traditional hierarchical
modulation suffers from the interference between layers, which
results in both low achievable capacity and high bit-error rate.
In this paper, an enhanced hierarchical modulation techniques
along with three optimization schemes are presented and discussed.
The proposed enhanced hierarchical modulation, in which the
enhancement-layer signal constellation is rotated, can help
achieve high performance with minimizing the inter-layer
interference. For achieving the performance, three schemes are
proposed from different perspectives of hierarchical modulation
and demodulation. The first scheme is straightforward. It is
proposed for maximizing the achievable spectral efficiency with
rotating the enhancement-layer signal constellation. The second
one is proposed for lowering the demodulation symbol-error rate
with maximizing the modulation efficiency, which is also proposed
by us for quantizing the effect of inter-layer interference. The
last approach is suggested for maximizing power amplifier
efficiency with reducing the peak-to-average-power ratio of
modulated signals. This approach is especially critical for
