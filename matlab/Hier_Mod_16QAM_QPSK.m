...
alpha = 1 ;
gamma = 0.00001:0.01:0.1 ;      %% The power splitting in terms of minimum Euclid distance
gamma_dB = -10:-1:-30 ;
    ...
    for n = 1 : 1 %length( beta )
        ...
        for r = 1 : length(SNR)
            ...
            for p = 1 : N_s
                ...
                E_Q(p) = mean( log2( sum( exp( - ( abs( s0(p) + w - s0 ).^2 - abs(w).^2 ) ./ (2*sigma^2) ) ) ) );
                
                
                for t = 1 : RUNS
                    
                    w = ( randn(N_s,1) + j*randn(N_s,1) ) * sigma ;
                    
                    PP(t) = log2( sum( exp( - ( abs( s0(p) + w - s0 ).^2 - abs(w).^2 ) ./ (2*sigma^2) ) ) );
