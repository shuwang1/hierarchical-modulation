...
beta  = 1:-0.2:0.00001 ; % 0.8: -0.8 : 0.000001 ; 
...
runs = ceil( ber_16QAM.^(-1) ).*100 ;

for m = 1 : length(SNR0_dB) 
    
    RUNS = min( runs( m ), 10e6 ) ;
    snr_dB = SNR0_dB( m ) ;    
...
for m = 1 : length(SNR_dB)
    ...
    for n = 1 : length(beta)
        for k = 1 : length(theta)
            ...
            r_QPSK  = awgn( s_QPSK,  snr_dB, 'measured' ) + s_enhan*sqrt( (1-beta(n))/beta(n) ) ;
...
save(  strcat( 'ESNR_QPSK_16QAM_', num2str(now), '.mat' ) ) ;
...
figure( 2005 )
semilogy( EbNo_dB0_QPSK, ber_QPSK, EbNo_dB0_QPSK, ber_QPSK_calc, '--' , EbNo_dB_QPSK , BER_QPSK(:, 1, 1) , '-.' , ...
    EbNo_dB0_16QAM, ber_16QAM, EbNo_dB0_16QAM, ber_16QAM_calc, '--'  ) ;
grid ;
ylim([10^(-7) 1])
xlabel( 'E_b/N_o' ) ;
ylabel( 'BER' ) ;
