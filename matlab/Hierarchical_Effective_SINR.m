...
EbNo_dB_QPSK  = SNR0_dB  - 10*log10( 2 ) ;
EbNo_dB_16QAM = SNR0_dB  - 10*log10( 4 ) ;

ber_QPSK            = berawgn( EbNo_dB_QPSK,  'qam' , 4  ) ;
ber_16QAM           = berawgn( EbNo_dB_16QAM, 'qam' , 16 ) ;
ber_QPSK_calculated = qfunc( sqrt(SNR0) ) ;

figure( 1000 )
semilogy( EbNo_dB_QPSK, ber_QPSK, EbNo_dB_QPSK, ber_QPSK_calculated, '--' , EbNo_dB_16QAM , ber_16QAM ) ;
grid ;
ylim([10^(-7) 1])
xlabel( 'E_b/N_o' ) ;
ylabel( 'BER' ) ;
...
figure( 1010 )
semilogy( SNR0_dB, ber_QPSK, SNR0_dB, ber_QPSK_calculated, '--' , SNR0_dB , ber_16QAM ) ;
grid ;
ylim([10^(-10) 1])
xlabel( 'E_s/N_o' ) ;
ylabel( 'BER' ) ;
...
for m = 1 : length(SNR_dB)
    ...
    for n = 1 : length(beta)
        for k = 1 : length(theta)
            ...
            r_QPSK  = awgn( s_QPSK,  snr_dB ) + s_enhan*sqrt( (1-beta(n))/beta(n) ) ;
            r_16QAM = awgn( s_16QAM, snr_dB, 'measured' ) + s_enhan*sqrt( (1-beta(n))/beta(n)*P_16QAM/P_QPSK );
            
            d_QPSK  = qamdemod( r_QPSK  ,  4 , 0 ) ;       
            d_16QAM = qamdemod( r_16QAM , 16 , 0 ) ;   
            
 
            [ num_QPSK( k, n, m ),  BER_QPSK( m, n, k )  ] = biterr( b_QPSK,  d_QPSK  ) ;
            [ num_16QAM( k, n, m ), BER_16QAM( m, n, k ) ] = biterr( b_16QAM, d_16QAM ) ;  
            
            [ num_QPSK( k, n, m ),  SER_QPSK( m, n, k )  ] = symerr( b_QPSK,  d_QPSK  ) ;            
            
            BER_theta( 1, k ) = BER_QPSK( m, n, k ) ;
            BER_theta( 2, k ) = BER_16QAM( m, n, k ) ;
            
            
        end
        
        [ BER_QPSK_min( m, n ), index ] = min( BER_theta( 1, : ) ) ;        
        theta_QSK( m , n ) = theta( index ) ; 
...
ESNR_QPSK           = qfuncinv( BER_QPSK ).^2 ;            
ESNR_dB_QPSK        = log10( ESNR_QPSK )*10 ;
ESNR_QPSK_min       = qfuncinv( BER_QPSK_min ).^2 ;
ESNR_dB_QPSK_min    = log10( ESNR_QPSK_min ).*10 ;

save(  strcat( 'Effective_SNR_QPSK_16QAM', num2str(now), '.mat' ) ) ;

figure( 2000 )
semilogy( EbNo_dB_QPSK, ber_QPSK, EbNo_dB_QPSK, ber_QPSK_calculated, '--' , EbNo_dB_QPSK, BER_QPSK(:, 1, 1) , '-.', EbNo_dB_QPSK, BER_QPSK(:, 2, 1) , '-.', EbNo_dB_QPSK, BER_QPSK(:, 3, 1) , '-.' , EbNo_dB_16QAM, ber_16QAM ) ;
grid ;
ylim([10^(-7) 1])
xlabel( 'E_b/N_o' ) ;
ylabel( 'BER' ) ;
...
figure( 2010 )
semilogy( SNR0_dB, ber_QPSK.*2, SNR0_dB, ber_QPSK_calculated.*2, '--' , SNR0_dB, SER_QPSK( :, 1, 1 ), '-o', SNR0_dB, SER_QPSK( :, 2, 1 ), '--') ;
grid ;
ylim([10^(-7) 1])
xlabel( 'E_s/N_o' ) ;
ylabel( 'SER' ) ;
...
figure( 2020 )
plot( SNR_dB, SNR_dB, SNR_dB , ESNR_dB_QPSK( :, 1, 1 ),'--',  SNR_dB ,ESNR_dB_QPSK( :, 2, 1 ),'--' ,SNR_dB , ESNR_dB_QPSK_min(:, 2), '-.',  SNR_dB ,ESNR_dB_QPSK( :, 3, 1 ),'--' ,SNR_dB , ESNR_dB_QPSK_min(:, 3), '-.' ) ;
xlim([0 20])
ylim([0 20])
grid ;
xlabel( 'E_s/N_o' ) ;
...
figure( 2030 )
plot( SNR_dB, 10.^( (ESNR_dB_QPSK( :, 1, 1 ) - SNR_dB')/10 ) , SNR_dB  , 10.^( (ESNR_dB_QPSK( :, 2, 1 ) - (SNR_dB )')/10 ), '--' ,  SNR_dB , 10.^( (ESNR_dB_QPSK_min(:, 2) - (SNR_dB )')/10 ),'-.' ,  SNR_dB, 10.^(( ESNR_dB_QPSK( :, 3, 1 ) - (SNR_dB )')/10 ), '--' ,  SNR_dB , 10.^(( ESNR_dB_QPSK_min(:, 3) - (SNR_dB )')/10 ),'-.' ) ;
xlim([0 20])
grid ;
xlabel( 'E_s/N_o' ) ;
ylabel( 'Effective E_s/N_o' ) ;
...
figure( 2040 )
plot( SNR_dB  , ESNR_dB_QPSK( :, 1, 1 ) - SNR_dB', SNR_dB  , ESNR_dB_QPSK( :, 2, 1 ) - (SNR_dB )', '--' ,  SNR_dB , ESNR_dB_QPSK_min(:, 2) - (SNR_dB )','-.' ,  SNR_dB, ESNR_dB_QPSK( :, 3, 1 ) - (SNR_dB )', '--' ,  SNR_dB , ESNR_dB_QPSK_min(:, 3) - (SNR_dB )','-.') ;
xlim([0 20])
grid ;
xlabel( 'E_s/N_o' ) ;
